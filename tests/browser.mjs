...
await page.locator('#registrar').fill('Chris');assert.equal(await page.locator('#registrarKo').inputValue(),'크리스');await page.locator('#nameMode').selectOption('korean');await page.locator('#registrar').fill('Daeyeol');assert.equal(await page.locator('#registrarKo').inputValue(),'대열');await page.locator('#nameMode').selectOption('english');await page.locator('#registrar').fill('Trevor Bergen');await page.locator('#sex').selectOption('F');await page.locator('#confirmed').check();assert.equal(await page.locator('#registrarKo').inputValue(),'트레버 버겐');assert.equal(await page.evaluate(()=>window.registeredTool.execute({}).completed),15);
